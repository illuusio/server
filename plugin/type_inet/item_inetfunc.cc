...
String *Item_func_inet6_aton::val_str(String *buffer)
{
  ...
  Inet6Bundle::Xyz_null ipv6(*tmp.string());
  if (!ipv6.is_null())
  {
...
String *Item_func_inet6_ntoa::val_str_ascii(String *buffer)
{
  ...
  Inet6Bundle::Xyz_null ipv6(static_cast<const Binary_string&>(*tmp.string()));
  if (!ipv6.is_null())
  {
...
class IP6 : public Inet6Bundle::Xyz_null
{
public:
  IP6(Item* arg) : Inet6Bundle::Xyz_null(arg) {}
...
longlong Item_func_is_ipv6::val_int()
{
  DBUG_ASSERT(fixed());
  String_ptr_and_buffer<STRING_BUFFER_USUAL_SIZE> tmp(args[0]);
  return !tmp.is_null() && !Inet6Bundle::Xyz_null(*tmp.string()).is_null();
}
