...
template<>
bool Inet6Bundle::Xyz::ascii_to_xyz(const char *str, size_t str_length)
{
  if (str_length < 2)
  {
...
template<>
size_t Inet6Bundle::Xyz::to_string(char *dst, size_t dstsize) const
{
  struct Region
  {
    int pos;
...
template<>
const Name &Inet6Bundle::Type_handler_xyz::default_value() const
{
  static Name def(STRING_WITH_LEN("::"));
  return def;
}

//template class PluginSimpleFixedBinaryType<IN6_ADDR_SIZE, IN6_ADDR_MAX_CHAR_LENGTH>;
